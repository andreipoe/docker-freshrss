...
# Note: You shouldn't make all FreshRSS files public. See https://github.com/FreshRSS/FreshRSS/tree/1.7.0#advices for more info.
...
RUN set -x && \
    ...
    rm -rf /var/www/html && \
    ! echo "${FRSS_HOME}" | egrep -q '^/var/www/html' && ln -s "${FRSS_HOME}/p" /var/www/html || echo "You should not set FRSS_HOME to /var/www/html. See https://github.com/FreshRSS/FreshRSS/tree/1.7.0#advices for more info." >&2 && \
